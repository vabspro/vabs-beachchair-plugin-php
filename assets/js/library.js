...
//As contains() isn't supported in Chrome
    ...
    let GetURLGetParameter = function (parameterName) {
        let result = null,
            tmp = [];
        location.search
            .substr(1)
    ...
    let HideElement = function (element, delay = 200) {
        element.hide(delay);
    }

    let ShowElement = function (element, delay = 200) {
        element.show(delay);
    }

    let SetOrReplaceURLParam = function (key, value) {
    ...
    let PopulateAndShowModal = function (response) {
    ...
    let Message = function (title, message) {
    ...
    let FormatToPrice = function (total) {
    ...
    let IsValidEmail = function (mail) {
